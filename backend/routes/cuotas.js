...
// Deja pasar solo los campos que se pueden cargar o editar desde el panel.
// Sin esto, el cuerpo del request entraba entero a Sequelize y se podía mandar,
// por ejemplo, un `id` propio para pisar otra cuota.
const CAMPOS_EDITABLES = [
  'alumna_id','mes','anio','tipo','monto','fecha_pago',
  'medio_pago','recibo_nro','observacion','comprobante','actividad_id',
]
function soloCamposEditables(body) {
  const limpio = {}
...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    const cuota = await Cuota.create(soloCamposEditables(req.body))
...
router.put('/:id', requireAuth, async (req, res) => {
  try {
    ...
    // Una profesora solo puede tocar cuotas de alumnas de sus grupos, igual que
    // en el POST. Antes acá no se validaba nada y podía editar cualquier cuota
    // del estudio, incluido el monto y a qué alumna pertenece.
    if (req.user.rol === 'profesora') {
      if (!req.user.profesora_id) return res.status(403).json({ error: 'Sin acceso' })
      const ids = await alumnaIdsDeProfe(req.user.profesora_id)
      if (!ids.includes(cuota.alumna_id))
        return res.status(403).json({ error: 'La alumna no pertenece a tu grupo' })
      if (req.body.alumna_id !== undefined && !ids.includes(Number(req.body.alumna_id)))
        return res.status(403).json({ error: 'La alumna no pertenece a tu grupo' })
    }

    await cuota.update(soloCamposEditables(req.body))
    res.json({ ok: true })
  } catch (err) {
    res.status(400).json({ error: err.message })
  }
